
public class hotelDTO {
private int booking_number;
private int room_number;
private int booked_people;
private String checkin_date;
private String checkout_date;
private int total_price;
private String booking_person;
private String mobile;
private String room_name;
private String type_name;
private int avail_people;
private int price;
private int room_type;

public hotelDTO() {
}
public int getBooking_number() {
	return booking_number;
}
public void setBooking_number(int booking_number) {
	this.booking_number = booking_number;
}
public int getRoom_number() {
	return room_number;
}
public void setRoom_number(int room_number) {
	this.room_number = room_number;
}
public int getBooked_people() {
	return booked_people;
}
public void setBooked_people(int booked_people) {
	this.booked_people = booked_people;
}
public int getTotal_price() {
	return total_price;
}
public void setTotal_price(int total_price) {
	this.total_price = total_price;
}
public String getBooking_person() {
	return booking_person;
}
public void setBooking_person(String booking_person) {
	this.booking_person = booking_person;
}
public String getMobile() {
	return mobile;
}
public void setMobile(String mobile) {
	this.mobile = mobile;
}
public String getCheckin_date() {
	return checkin_date;
}
public void setCheckin_date(String checkin_date) {
	this.checkin_date = checkin_date;
}
public String getCheckout_date() {
	return checkout_date;
}
public void setCheckout_date(String checkout_date) {
	this.checkout_date = checkout_date;
}
public String getRoom_name() {
	return room_name;
}
public void setRoom_name(String rooom_name) {
	this.room_name = rooom_name;
}
public String getType_name() {
	return type_name;
}
public void setType_name(String type_name) {
	this.type_name = type_name;
}
public int getAvail_people() {
	return avail_people;
}
public void setAvail_people(int avail_people) {
	this.avail_people = avail_people;
}
public int getPrice() {
	return price;
}
public void setPrice(int price) {
	this.price = price;
}
public int getRoom_type() {
	return room_type;
}
public void setRoom_type(int room_type) {
	this.room_type = room_type;
}




}
